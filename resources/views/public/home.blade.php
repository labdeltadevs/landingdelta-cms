<x-layouts::public>
    ...
    <section class="relative min-h-[90vh] flex items-center overflow-hidden"
             style="background-image:
                 linear-gradient(to bottom, rgba(50,50,50,0.25), rgba(50,50,50,0.25), rgba(50,50,50,0.25)),
                 ...
                 url('{{ Storage::disk('public')->url('fondo-forth.jpg') }}');
             background-size: cover, cover, cover, 28px 28px, cover;
        ...
        <div class="relative z-10 mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-12 md:py-20">
            ...
            <div class="rounded-2xl bg-black/30 backdrop-blur-lg border border-white/[0.08] p-6 sm:p-8 lg:p-12 shadow-2xl shadow-black/40">
            ...
            <div class="flex flex-col lg:flex-row lg:items-center gap-8 lg:gap-16">
                ...
                <div class="flex-1 flex flex-col justify-center">
                    ...
                    @if ($slides->isNotEmpty())
                        <div x-data="{ current: 0, total: {{ $slides->count() }} }" x-init="setInterval(() => { current = (current + 1) % total }, 6000)" class="mb-8">
                            <template x-for="(slide, i) in {{ json_encode($slides->map(fn($s) => ['title' => $s->title, 'subtitle' => $s->subtitle, 'cta_label' => $s->cta_label, 'cta_url' => $s->cta_url])) }}" :key="i">
                                <div x-show="current === i"
                                    ...
                                    <h1 class="text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-bold text-white leading-tight tracking-tight drop-shadow-lg" x-text="slide.title"></h1>
                                    <p class="mt-4 text-base sm:text-lg lg:text-xl text-zinc-200/80 leading-relaxed" x-text="slide.subtitle"></p>
                            ...
                            <div class="flex gap-2 mt-6">
                                <template x-for="(_, i) in total" :key="i">
                                    <button @click="current = i"
                                            ...
                                            :class="current === i ? 'w-8 bg-[#ff671f] shadow-[0_0_10px_rgba(255,103,31,0.5)]' : 'w-1.5 bg-white/30 hover:bg-white/50'"></button>
                                </template>
                            </div>
                        </div>
                    @endif
                    ...
                    <div data-aos="fade-up" data-aos-delay="200" class="mb-10">
                        <div class="flex items-center gap-4 mb-5">
                            ...
                            <span class="text-[11px] font-semibold uppercase tracking-[0.25em] text-[#ff671f] drop-shadow-sm">
                                Lider en la industria farmacéutica Boliviana
                            </span>
                        </div>
                        <p class="text-md sm:text-md lg:text-md text-white/95 leading-relaxed max-w-2xl font-light drop-shadow-sm">
                    ...
                    <div class="flex flex-wrap gap-4" data-aos="fade-up" data-aos-delay="300">
                        <a href="{{ route('public.products.index') }}"
                           class="group inline-flex items-center gap-2.5 rounded-full bg-[#ff671f] px-8 py-4 text-sm font-semibold text-white shadow-lg shadow-orange-500/25 transition-all duration-300 hover:bg-[#e55a1a] hover:shadow-orange-500/50 hover:-translate-y-1 hover:scale-105">
                            Explorar productos
                            <svg class="h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3"/></svg>
                        </a>
                        <a href="{{ route('public.about') }}"
                           class="group inline-flex items-center gap-2.5 rounded-full border border-white/25 bg-white/[0.12] backdrop-blur-md px-8 py-4 text-sm font-semibold text-white transition-all duration-300 hover:bg-white/[0.20] hover:border-white/50 hover:-translate-y-1 hover:scale-105">
                            Nuestra historia
                            <svg class="h-4 w-4 opacity-60 transition-transform duration-300 group-hover:translate-x-1" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17.25 8.25 21 12m0 0-3.75 3.75M21 12H3"/></svg>
                        </a>
                    </div>
                </div>
                ...
                <div class="flex-shrink-0 flex flex-col justify-center" data-aos="zoom-in" data-aos-delay="400">
                    <div class="relative pl-8 border-l-2 border-white/10">
                        @php $galardones = [
                            ['icon' => '<svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>', 'label' => '30+ años', 'sub' => 'de experiencia'],
                            ['icon' => '<svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285Z"/></svg>', 'label' => 'Orgullo', 'sub' => 'boliviano'],
                            ['icon' => '<svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 10.5a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/><path stroke-linecap="round" stroke-linejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1 1 15 0Z"/></svg>', 'label' => 'Eje', 'sub' => 'troncal']
                        ]; @endphp
                        @foreach ($galardones as $g)
                            <div class="relative pb-10 last:pb-0 group"
                                 data-aos="fade-left" data-aos-delay="{{ 500 + $loop->index * 100 }}">
                                {{-- Timeline connector dot --}}
                                <div class="absolute -left-[5px] top-6 w-2.5 h-2.5 rounded-full bg-[#ff671f] border-2 border-white/20 shadow-[0_0_12px_rgba(255,103,31,0.4)] transition-all duration-500 group-hover:shadow-[0_0_20px_rgba(255,103,31,0.6)] group-hover:scale-125"></div>
                                ...
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="pt-1">
                                        <span class="block text-sm font-bold text-white leading-tight drop-shadow-sm">{{ $g['label'] }}</span>
                                        <span class="block text-xs text-white/80 mt-0.5 drop-shadow-sm">{{ $g['sub'] }}</span>
                                    </div>
                                </div>
                            </div>
                        @endforeach
            ...
            <div class="mt-10 lg:mt-12 grid grid-cols-3 divide-x divide-white/10 rounded-2xl bg-black/50 backdrop-blur-lg border border-white/[0.08] overflow-hidden"
                ...
                <div class="py-5 text-center group">
                    <p class="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#ff671f] transition-all duration-300 group-hover:scale-110">{{ $productsCount }}+</p>
                    <p class="text-xs sm:text-sm text-white/80 mt-1 font-medium tracking-wide drop-shadow-sm">Productos</p>
                </div>
                <div class="py-5 text-center group">
                    <p class="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#ff671f] transition-all duration-300 group-hover:scale-110">{{ $brands->count() }}</p>
                    <p class="text-xs sm:text-sm text-white/80 mt-1 font-medium tracking-wide drop-shadow-sm">Marcas</p>
                </div>
                <div class="py-5 text-center group">
                    <p class="text-2xl sm:text-3xl lg:text-4xl font-bold text-[#ff671f] transition-all duration-300 group-hover:scale-110">{{ $branches->count() }}</p>
                    <p class="text-xs sm:text-sm text-white/80 mt-1 font-medium tracking-wide drop-shadow-sm">Sucursales</p>
                </div>
            </div>
    ...
    @if ($brands->isNotEmpty())
        <section class="relative overflow-hidden bg-gradient-to-b from-zinc-50 to-white py-24"
                 x-data="{
                     ...
                     prev() { this.current = (this.current - 1 + this.total) % this.total; clearInterval(this.autoplay); this.startAutoplay(); },
                     next() { this.current = (this.current + 1) % this.total; clearInterval(this.autoplay); this.startAutoplay(); },
                     goTo(i) { this.current = i; clearInterval(this.autoplay); this.startAutoplay(); },
                ...
                <div class="text-center mb-12">
                    <span class="inline-block text-[11px] font-semibold uppercase tracking-[0.2em] text-[#ff671f] mb-3">Distribución exclusiva</span>
                    <h2 class="text-3xl sm:text-4xl font-bold text-zinc-900">Nuestras <span class="text-[#ff671f]">Marcas</span></h2>
                    <p class="mt-3 text-sm text-zinc-500 max-w-lg mx-auto">Conoce las marcas que representamos y distribuimos en todo Bolivia.</p>
                </div>

                {{-- Carousel with real before/after peek --}}
                <div class="relative mx-auto max-w-4xl px-4 sm:px-6" @mouseenter="clearInterval(autoplay)" @mouseleave="startAutoplay()">
                    ...
                    <div class="relative min-h-[340px] sm:min-h-[380px] flex items-center justify-center">
                        @foreach ($brands as $idx => $brand)
                            ...
                            <div x-show="{{ $idx }} === prevIndex"
                                ...
                                <div class="bg-white/40 backdrop-blur-sm rounded-2xl border border-white/10 p-4 text-center opacity-50 scale-90">
                                    @if ($brand->logo_path)
                                        <img src="{{ $brand->logo_url }}" alt="" class="h-12 w-12 sm:h-16 sm:w-16 mx-auto object-contain" />
                                    @else
                                        <div class="h-12 w-12 sm:h-16 sm:w-16 mx-auto rounded-xl bg-gradient-to-br from-[#ff671f]/10 to-[#ff671f]/5 flex items-center justify-center text-lg sm:text-2xl font-bold text-[#ff671f]/40">{{ substr($brand->name, 0, 1) }}</div>
                                    @endif
                            ...
                            <div x-show="{{ $idx }} === current"
                                ...
                                <div class="bg-white/70 backdrop-blur-xl rounded-2xl border border-white/20 p-6 sm:p-8 shadow-xl shadow-black/5 text-center">
                                    <div class="flex justify-center mb-4">
                                        @if ($brand->logo_path)
                                            <img src="{{ $brand->logo_url }}" alt="{{ $brand->name }}" class="h-20 w-20 sm:h-28 sm:w-28 object-contain" />
                                        @else
                                            <div class="h-20 w-20 sm:h-28 sm:w-28 rounded-2xl bg-gradient-to-br from-[#ff671f]/10 to-[#ff671f]/5 flex items-center justify-center text-3xl sm:text-4xl font-bold text-[#ff671f]/60">{{ substr($brand->name, 0, 1) }}</div>
                                        @endif
                                    </div>
                                    <h3 class="text-lg sm:text-xl font-bold text-zinc-900">{{ $brand->name }}</h3>
                                    @if ($brand->description)
                                        <p class="mt-2 text-sm text-zinc-500 leading-relaxed max-w-xs mx-auto">{{ $brand->description }}</p>
                                    @endif
                                    <a href="{{ route('public.brands.show', $brand) }}" class="mt-5 inline-flex items-center gap-2 rounded-full bg-[#ff671f] px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-orange-500/20 transition-all hover:bg-[#e55a1a] hover:-translate-y-0.5">
                                        Ver productos
                ...
                <div class="flex justify-center gap-2 mt-8" data-aos="fade-up" data-aos-delay="200">
                    @foreach ($brands as $i => $brand)
                        <button @click="goTo({{ $i }})"
                                ...
                                :class="current === {{ $i }} ? 'w-8 bg-[#ff671f] shadow-[0_0_8px_rgba(255,103,31,0.4)]' : 'w-2 bg-zinc-300 hover:bg-zinc-400'"></button>
                    @endforeach
                </div>
            </div>
        </section>
